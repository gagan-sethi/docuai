...
export default function MergePreviewModal({
  ...
  const headline = useMemo(
    () => (cleaned ? pickHeadlineTotal(cleaned.totals) : null),
    [cleaned]
  );

  const filteredRows = useMemo(() => {
    if (!cleaned) return [];
    if (!search.trim()) return cleaned.rows;
  ...
  const handleDownload = () => {
    if (!cleaned) return;
    const csv = serializeCleanedCsv(cleaned);
          ...
          >
            ...
            <div className="flex-shrink-0 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-emerald-100">
                      ...
                      merged · {cleaned.headers.length} unified columns ·{" "}
                      {cleaned.rowCount} row{cleaned.rowCount !== 1 ? "s" : ""}
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-emerald-700/60 border-t border-emerald-700/60">
                ...
                <Stat
                  ...
                  label="Unified columns"
                  value={String(cleaned.headers.length)}
                  hint={
                    cleaned.droppedColumns.length > 0
                      ? `${cleaned.droppedColumns.length} empty dropped`
                      : "no empty columns"
                  }
                />
                <Stat
                ...
                <Stat
                  ...
                  label={headline ? headline.header : "Total"}
                  value={headline ? formatAmount(headline.value) : "—"}
                  emphasise
                />
              </div>
            </div>
            ...
            <div className="flex-shrink-0 flex flex-wrap items-center gap-3 px-6 py-3 border-b border-slate-100 bg-slate-50/50">
              ...
              <span className="text-xs text-slate-500">
                Showing {filteredRows.length} of {cleaned.rowCount} row
                {cleaned.rowCount !== 1 ? "s" : ""}
              </span>
              ...
              <span className="hidden sm:inline-flex items-center gap-1.5 text-[11px] font-medium text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-full px-2.5 py-1">
                ...
                Cleaned · no empty cells
              </span>
              <button
                onClick={handleDownload}
                className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-white bg-gradient-to-r from-primary to-primary-dark rounded-lg shadow-sm hover:shadow-md transition-all"
              >
              ...
              <table className="w-full text-xs border-separate border-spacing-0">
                ...
                <tbody>
                  {filteredRows.length === 0 && (
                    <tr>
                      <td
                        colSpan={cleaned.headers.length + 1}
                ...
                {Object.keys(cleaned.totals).length > 0 && (
            ...
            <div className="flex-shrink-0 px-6 py-3 border-t border-slate-100 bg-slate-50/80 flex flex-wrap items-center gap-3 text-xs text-slate-500">
              ...
              {cleaned.droppedColumns.length > 0 && (
                <span className="inline-flex items-center gap-1.5">
                  ...
                  Dropped {cleaned.droppedColumns.length} empty column
                  {cleaned.droppedColumns.length !== 1 ? "s" : ""}:{" "}
                  <span className="text-slate-400 truncate max-w-[260px]">
                    {cleaned.droppedColumns.slice(0, 4).join(", ")}
                    {cleaned.droppedColumns.length > 4 ? "…" : ""}
                  </span>
                </span>
              )}
              <div className="flex-1" />
              ...
              <button
                onClick={handleDownload}
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition"
              >
